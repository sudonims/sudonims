...
const CaretJS = {
  ...
  update(e) {
    let input = e.target,
      ...
      max = input.hasAttribute('maxlength')
    ...
    if (e.type === 'keydown' && !e.ctrlKey) {
      if (
        (CaretJS.var.char_keys.indexOf(e.code) > -1 &&
          (max === -1 || pos < max)) ||
        (e.code === 'ArrowRight' &&
    ...
    document.querySelector('#' + input.id + ' + span').innerHTML =
      '&nbsp;'.repeat(pos + diff) + '<span>█</span>';
  },
    ...
    window.onload = () => {
      for (var id in CaretJS.carets) {
        ...
        document.querySelector(
          '#' + CaretJS.carets[id].input.id + ' + span'
        ).innerHTML = '<span>█</span>';
      }
    };
  },
